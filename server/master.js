...
module.exports = async () => {
  ...
  // Crear y asignar manualmente el servidor HTTP
  WIKI.server = http.createServer(WIKI.app);

  WIKI.server.listen(WIKI.config.port, () => {
    console.log(`Servidor escuchando en el puerto ${WIKI.config.port}`);
  });

  if (WIKI.server) {
    WIKI.server.setTimeout(300000); // Aumenta el timeout a 5 minutos
    console.log('Timeout configurado correctamente.');
  } else {
    console.error('WIKI.server sigue sin estar definido.');
  }
